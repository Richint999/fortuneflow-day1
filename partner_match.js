...
function score(A,B,det){const da=dom(A),db=dom(B);let comp=0;if(da.least===db.most)comp+=22;if(db.least===da.most)comp+=10;if((B[da.least]||0)-(A[da.least]||0)>=2)comp+=8;const union={};E.forEach(k=>union[k]=(A[k]||0)+(B[k]||0));const vals=E.map(k=>union[k]||0),avg=vals.reduce((a,b)=>a+b,0)/vals.length;const v=vals.reduce((s,x)=>s+Math.pow(x-avg,2),0)/vals.length;const balance=Math.max(0,25*(1/(1+v)));let pair=brZh(det.a.db)+brZh(det.b.db),rev=pair.split('').reverse().join('');let rel=0;if(SIX_COMB.has(pair)||SIX_COMB.has(rev))rel+=12;if(SIX_HARM.has(pair)||SIX_HARM.has(rev))rel+=6;TR.forEach(s=>{ if(s.has(brZh(det.a.db)) && s.has(brZh(det.b.db))) rel+=5; });const stemRel=cycleRel(eStem(det.a.ds),eStem(det.b.ds));const stemScore=stemRel===1?8:stemRel===-1?-6:2;const totalScore=Math.round(Math.max(0,Math.min(100,comp+balance+rel+stemScore)));return { total: totalScore, detail:{comp, balance:Math.round(balance), rel, stemScore}, domA:da, domB:db };}
