// Partner Match – fixed, self-contained script (drop-in replacement)
...
function dom(counts){
  ...
  const most=items[0][0], least=items[items.length-1][0];
  const avg=items.reduce((s,v)=>s+v[1],0)/items.length;
  const v=items.reduce((s,v)=>s+Math.pow(v[1]-avg,2),0)/items.length;
...
function monthPillar(yStem,dateUTC){
  ...
  const S={1:4,2:6,3:5,4:6,5:6,6:7,7:8,8:8,9:8,10:7,11:7,12:6}; // approx solar terms
  let k=0;
...
function cycleRel(a,b){
  ...
  if((ai+1)%5===bi) return 1; // a -> b (generates)
  if((ai+4)%5===bi) return -1; // a <- b (controls)
  return 0;
}

// Convert date to pillars (local civil date, optional hour)
...
function score(A,B,det){
  ...
  let pair=brZh(det.a.db)+brZh(det.b.db), rev=pair.split('').reverse().join('');
  let rel=0;
...
// Expose functions for page script if needed
window.__PM__ = { buildPillars, countsFromPillars, score };
